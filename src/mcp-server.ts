...
`;
  ...
  private setupTools(): void {
    ...
    this.server.tool(
      'write_terminal',
      'Write input to a terminal session. For Codex/LLM chat: send message text first, then force Enter with sendEnter=true when the app appears to wait for submission.',
      {
      ...
      async ({ terminalId, input, appendNewline, sendEnter }): Promise<CallToolResult> => {
        try {
    ...
    this.server.tool(
      'read_terminal',
      'Read output from a terminal session. For Codex/TUI conversation, use mode="tail" + raw=true + cleanAnsi=true. If tail seems incomplete, retry with mode="head-tail" in raw mode.',
      {
        terminalId: z.string().describe('Terminal session ID'),
...
The more detailed, the better the fix!`),
  ...
  private resolveReadMaxChars(maxChars?: number): number {
  ...
  private applyModeToRawText(
    ...
    const safeHead = Number.isFinite(options.headLines) && (options.headLines || 0) > 0
      ? Math.floor(options.headLines as number)
      : 50;
    const safeTail = Number.isFinite(options.tailLines) && (options.tailLines || 0) > 0
      ? Math.floor(options.tailLines as number)
      : 50;
  ...
  private truncateForContext(output: string, maxChars: number): {
    output: string;
    truncated: boolean;
    omittedChars: number;
  } {
  ...
  private sanitizeRawTerminalOutput(raw: string): string {
    if (!raw) {
      return raw;
    }
  ...
  private skipAnsiSequence(input: string, startIndex: number): number {
...
Would you like specific help with your issue?`
    ...
    // Codex/TUI 使用最佳实践提示
    this.server.prompt(
      'codex-tui-best-practices',
      'Best practices for reading Codex/TUI output and sending Enter correctly',
      {
        terminalId: z.string().optional().describe('Optional terminal ID for concrete command examples')
      },
      async ({ terminalId }): Promise<GetPromptResult> => {
