...
`;
    ...
    setupTools() {
        ...
        this.server.tool('create_terminal', 'Create a new persistent terminal session', {
            ...
            readOnlyHint: false
        }, async ({ shell, cwd, env, initCommands, readyPattern, readyTimeoutMs, initFailurePattern, statusFile }) => {
            try {
        ...
        this.server.tool('read_terminal', 'Read output from a terminal session. For Codex/TUI conversation, use mode="tail" + raw=true + cleanAnsi=true. If tail seems incomplete, retry with mode="head-tail" in raw mode.', {
            ...
            readOnlyHint: true
        }, async ({ terminalId, since, maxLines, mode, headLines, tailLines, stripSpinner, raw, cleanAnsi, maxChars, adapter }) => {
            try {
        ...
        this.server.tool('get_terminal_status', 'Get a structured status snapshot of a terminal session including process state, semantic status, command info, and buffer cursors.', {
            terminalId: z.string().describe('Terminal session ID'),
        ...
        this.server.tool('wait_for_pattern', 'Wait for a specific pattern to appear in terminal output. Returns when pattern matches or timeout expires. Useful for detecting command completion, prompt appearance, or structured output.', {
            terminalId: z.string().describe('Terminal session ID'),
...
The more detailed, the better the fix!`),
        ...
        this.server.tool('wait_for_result', 'Wait for a structured <task_result> XML block in terminal output and parse it. The XML must have a <status> element with value PASS, FAIL, or ERROR.', {
            terminalId: z.string().describe('Terminal session ID'),
            timeoutMs: z.number().optional().describe('Maximum wait time in milliseconds (default: 60000)'),
            since: z.number().optional().describe('Start reading from this sequence number')
        }, { title: 'Wait for Result', readOnlyHint: true }, async ({ terminalId, timeoutMs, since }) => {
            try {
                ...
                if (parseResult.parsed) {
                    return {
                        ...
                        structuredContent: parseResult.parsed
                    };
                }
                return {
                    content: [{ type: 'text', text: `Found <task_result> XML but failed to parse: ${parseResult.errors.map(e => e.message).join('; ')}` }],
                    isError: true
                };
            }
            catch (error) {
                return {
                    content: [{ type: 'text', text: `Error waiting for result: ${error.message}` }],
                    isError: true
                };
            }
        });
        // Resume terminal tool
        this.server.tool('resume_terminal', 'Resume a CLI agent session in a new terminal. Creates a new PTY and runs the resume command (e.g., claude --resume <session-id>). This is NOT PTY resurrection - a new process is created.', {
            ...
            readyTimeoutMs: z.number().optional().describe('Timeout for ready pattern in ms (default: 30000)')
        }, { title: 'Resume Terminal', readOnlyHint: false }, async ({ sessionId, cwd, shell, initCommands, readyPattern, readyTimeoutMs }) => {
            try {
                const resumeOpts = { sessionId };
                if (cwd)
                    resumeOpts.cwd = cwd;
                if (shell)
                    resumeOpts.shell = shell;
                if (initCommands)
                    resumeOpts.initCommands = initCommands;
                if (readyPattern)
                    resumeOpts.readyPattern = readyPattern;
                if (readyTimeoutMs)
                    resumeOpts.readyTimeoutMs = readyTimeoutMs;
                ...
                return {
                    content: [{ type: 'text', text: `Error resuming terminal: ${error.message}` }],
                    isError: true
                };
            }
        });
